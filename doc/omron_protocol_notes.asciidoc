...
All commands from the computer to the Omron device consist of at least
...
==== Description ====
Used to retrieve how many sessions are currently stored on the machine
...
Bytes B and C are not sent on HEM-637, so the return is only 15 bytes long.
...
Byte 5:: Seconds
Byte 6:: "Unit Number"
Byte 7:: High byte of SYS (usually 0)
Byte 8:: SYS
Byte 9:: DIA
Byte A:: Pulse
Byte B:: High byte of Status Flags (0)
Byte C:: Status Flags 
	 - 0x0X - Standalone reading 
	 - 0x01 - "Irregular heartbeat detected during measurement"
	 - 0x02 - "Movement detected during measurement"
